...
さっきまで「トマト」「牛乳」って表示されてたのに、**商品名がどこにも表示されてへん**やろ？ `{{ item.product_name }}` を読んでたテンプレートは、API のレスポンスに `product_name` がなくなった（`name` に変わった）から、`item.product_name` が `undefined` になっただけや。詳細画面も開いてみい。見出しも空っぽや。
...
「TS は interface しか見てへん」っちゅうことを、また「カラム名変更」で確認するで。冒頭のトラブルをもう一回再現するんや。
