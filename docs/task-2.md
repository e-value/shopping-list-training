...
このドキュメント画面では、**ブラウザから直接 API にリクエストを送れる** んや。今まで Postman とか使ってた人は、もうこれで済むで:

1. 左サイドバーの **`items.index`**（GET）をクリック
2. 右側にある **「Send API Request」** ボタンを押す
3. Response 欄に、実際の買い物リストデータが JSON で返ってくる

🙋 「おお！実際のデータが返ってきました！`product_name` も `quantity` も入ってる！」

🐘 「せやろ？Postman を別で起動せんでも、この画面だけで API の動作確認ができるんや」

🙋 「POST とか PUT もここからテストできるんですか？」

🐘 「できるで。例えば `items.store`（POST）を開くと、リクエストボディを入力する欄が出てくる。そこに `product_name` や `quantity` を入れて送ったら、実際にアイテムが作成されるんや。DELETE も同じ要領で消せる。**CRUD 操作のテストが全部この画面で完結する** っちゅうことや」
